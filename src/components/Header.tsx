...
export default function Header() {
  ...
  const getNavLinkClass = ({ isActive }: { isActive: boolean }) =>
    isActive
      ? 'bg-green-900 text-white font-bold px-3 py-2 rounded-lg transition duration-150 border border-green-900'
    ...
    if (user) {
      return (
        <Button
          ...
          className="inline-flex items-center justify-center whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium text-muted transition hover:bg-slate-100 hover:text-text"
        >
      ...
      <Container>
        <div className="flex h-16 items-center justify-between">
          ...
          <nav
            ...
            className="ml-auto mr-3 hidden items-center space-x-1 text-sm font-medium md:flex"
          >
        ...
        <Container className="space-y-3 pt-2 pb-4">
          <nav aria-label="Mobile navigation" className="space-y-1">
            <NavLink
              ...
              onClick={closeMobileMenu}
              className={(state) => `block ${getNavLinkClass(state)}`}
            >
              Home
            </NavLink>
            <NavLink
